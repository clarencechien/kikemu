...
本 repo 的五個實驗(見 `results/report.md`)直接決定了 app 的每一條技術路線:
...
4. 結束後,整場字幕留在**手機本地**(IndexedDB),可回看、可**逐場匯出**
   (MD / TXT / CSV,皆含原文與譯文)、可清除;伺服器不留任何內容——
...
- **歷史逐場匯出**:每筆紀錄有 MD / TXT / CSV 三顆鈕,三種都含原文與譯文,
  差別是**給誰讀**:MD 給 LLM(metadata 條列 + 一張對照表格,欄位對應最不會解錯,
  適合叫它摘要或整理成遊記)、TXT 給人(對照式,直接貼進筆記)、
  CSV 給試算表(兩欄 + BOM + CRLF,Excel 不會亂碼)。
  譯文缺漏的句子一律標「(未翻出)」,不靜默略過。
...
5. ~~**使用者的網路出口落在 Google 不服務的地區**~~ → **2026-09-25 實際發生並已修**:
   台灣行動網路被路由到 HKG,`SessionRelay` DO 建在那裡,Gemini 回 400
   `User location is not supported`。修法是 `GeminiProxy`(釘在 `wnam` 的代打 DO,
   只在被拒後啟動)。這是**實作前沒寫下來的風險**——「執行位置跟著使用者走」
   對任何有地區政策的上游都成立,之後接新供應商要先問這一題。
