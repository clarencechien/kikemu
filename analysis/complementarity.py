...
SUBSETS = [
    ...
    # handoff-v10:Scribe 進來之後,非即時那組的最佳配對可能變了。
    # 它們在 exp1 的強弱**正好相反**(N0 Gemini 0.940 / Scribe 0.866;
    # N3 Gemini 0.567 / Scribe 0.657),所以有理由懷疑錯的不是同一批詞。
    ("exp1", "非即時新配對", ["Abat", "Sbat_ja_full"],
     "Gemini 批次 + Scribe 批次。兩者總分幾乎相同(0.827 / 0.824)但"
     "噪音下的強弱相反,是雙線最有機會的一組。"),
    ("exp1", "非即時三方", ["Abat", "Sbat_ja_full", "Cbplus"],
     "再加上 SM 批次+詞表。三個都拿得到詞級 timestamp 或不需要。"),
    ("exp1", "Scribe 內部對照", ["Sbat_ja_full", "Srt_ja"],
     "同廠商的批次 vs 即時。§2.1e 證實那是兩顆不同的模型,"
     "所以這組的增益**不是**「同引擎抖動」,而是真的模型差異。"),
    ("exp5", "乾淨三方(都不與參考同源)", ["Xbrz_auto", "Xbat_bi", "Sbat_bi"],
     "Breeze + SM + Scribe。exp5 唯一完全避開主場偏差的三方組合。"),
]
