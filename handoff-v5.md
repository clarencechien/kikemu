...
# 附錄:Phase A 執行後的更新(2026-08-18)

## 1. 訓練資料污染已證實,任務書 §7 的偏誤警告要升級

Breeze ASR 25 模型卡的訓練資料:ODC Synth 10,000hr(合成中文)+
CommonVoice17-EN 1,738hr + **NTUML2021 11hr(真實中英夾雜)**。

`NTUML2021` 就是 S1 / S2 的來源課程。任務書 §7 寫的是「致謝含李宏毅教授,
團隊對此語域熟悉,表現可能偏高」——實際情況更直接:**模型在評測語料上訓練過**。

因此 §7 的處理方式 2(補領域外語料)從「建議」升級為「**必要條件**」,
而處理方式 3 要再收緊:S1/S2 連「領域內表現優異」都不能說,
只能說「在訓練過的語料上表現優異」,那不是一個有資訊量的陳述。

## 2. Phase A 結果摘要(詳見 results/report.md §3D)

tolerant 召回:X-breeze 0.949 / 0.780 / 0.875(S1-M0 / S1-M3 / S3-M0),
三檔皆高於所有 SM arm;M3 上 +0.24 贏過「雙語 pack + 投影片詞表」。
**預先登記的「噪音下會崩」沒有發生。**

兩個必須一起講的但書:
- M3 有一段 `,000,000,000` 重複 167 次的迴圈(Whisper 老毛病),
  收掉後 TER 0.380 → 0.261。**召回率不受影響**(重複內容是數字)。
- S1-M0 贏 Gemini Live、S3-M0 輸 Gemini Live —— 與污染假說一致。

## 3. `language` 參數:無差異,原因已查明

`language=None` 與 `"zh"` 輸出位元完全相同。模型 `generation_config.json` 的
`forced_decoder_ids` 是 `[[1, None], [2, 50359]]`:語言槽留給自動偵測、
task 固定 transcribe,而這批音訊自動偵測本來就判成中文。
notebook 已加健全性檢查(兩者相同時自動跑一次 `language="en"` 確認參數有效)。

## 4. 領域外語料的選擇條件(取代 §10.3 的「晚一步決定」)

必要條件:
1. **台灣華語 + 英文術語自然夾雜**(晶晶體),不是照稿唸
2. **不是台大 ML 相關課程**,最好連講者都換掉
3. **不是 ASCEND / CSZS-zh-en** —— 那兩個是 Breeze 模型卡自己報的
   code-switching benchmark,拿它們測等於用開發者最佳化過的靶
4. 授權可查、可長期取用
5. 有辦法建可信正解(有官方字幕最好,否則沿用 exp2 的三路轉寫 + 人工裁決)

首選:**COSCUP / SITCON / g0v 的議程錄影**——授權明確(COSCUP 明文
CC BY-SA 4.0)、講者眾多、語域是 DevOps/資安/前端而非 ML、
現場口語且術語密集,正是要驗的那條軸。
