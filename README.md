# kikemu きけむ

**聞く(きく,聽)+ む。** 打開 kikemu,它替你聽外語導覽,台灣正體字幕即時浮現——
神社的神名、茶室的典故,一個都不漏。

與 [manemu](https://manemu.ai-apps.work)(紅,說)、sukemu(青,看)同一命名與產品家族:
**kikemu 是琥珀(聽)**。

- 產品:https://kikemu.ai-apps.work (封測中)
- 這個 repo 同時是**產品本體**(`app/`)與**五個引擎選型實驗 + 一份 oracle 天花板分析**
  (`corpus/`、`exp2/`、`exp5/`、`analysis/`、`results/`)。
  架構的每一條決策都指得出是哪一份數據決定的。

---

## TL;DR — 2026-08-20:ElevenLabs Scribe 補測(即時 + 非即時 × 三種語言)

**不到 $1 測完做字幕的業界預設引擎。結論是:非即時線換人,即時線不動。**

### 一句話

**Scribe 的批次很強(日文 0.824、中英夾雜 0.921,都並列第一,而且吵的條件全場最高),
但它的即時是另一顆比較弱的模型(引擎輸 SM −0.075)。**

### 非即時:分岔點從「要不要 timestamp」換成「吵不吵」

以前的規則是「不需要 timestamp 就 Gemini 直出,需要就犧牲準確度用 SM 批次」。
**那個取捨沒了**——Scribe 批次同時有詞級 timestamp 與 Gemini 級準確度,還更便宜:

| | 日文 exp1 | N0 乾淨 | N3 人群 8dB | 詞級 timestamp | $/hr |
|---|---|---|---|---|---|
| Gemini 批次 | 0.827 | **0.940** | 0.567 | ❌ | $0.26 |
| **Scribe v2 批次** | **0.824** | 0.866 | **0.657** | ✅ | **$0.22** |
| SM 批次 + 完整詞表+假名 | 0.791 | 0.866 | 0.597 | ✅ | $0.24 |

→ **乾淨用 Gemini、吵用 Scribe、要 diarization 才用 SM。**
決策樹已改版([`stt-matrix.md`](results/stt-matrix.md))。

中英夾雜那格更值得看:**Scribe 0.921 完全沒給詞表**,與 Gemini 3.7 同分——
但 exp5 的參考是 Gemini 產的,**Gemini 系有主場偏差,Scribe 沒參與造參考**。

### 即時:主線不動,而且原因不是詞表

| arm(exp1 日文,即時) | 全部 | N3 |
|---|---|---|
| **SM 即時 + 完整詞表+假名(現行產線)** | **0.791** | **0.627** |
| SM 即時 + 50 詞(**詞表對齊控制組**) | 0.699 | 0.478 |
| Scribe v2 Realtime + 50 詞 | 0.624 | 0.343 |

**A − B = −0.075**:**詞表對齊之後 Scribe 的即時引擎還是輸**,不是被 50 詞上限害的。

> **而批次那格是 +0.096(Scribe 較強)——方向相反。**
> `scribe_v2` 與 `scribe_v2_realtime` 是**兩顆不同的模型**,而即時那顆比較弱。
> 這是「批次好不代表即時好」的**第二個廠商實例**(第一個是 Gemini,
> 批次 0.567 / Live 0.030)。**兩次都是即時那顆較弱。**

還有一個節奏問題:**SM 每 0.6 秒定稿一次,Scribe 每 13.7 秒** ——
文字要 14 秒才凍結,兩層時間軸接不上。⚠️ 但那是 VAD 預設值,API 可調而我們沒調。

### 關於「引擎切換鈕」

數據支持的**不是讓使用者選引擎,而是照場景固定**:

| 場景 | 引擎 | 為什麼 |
|---|---|---|
| **日文即時**(本業) | **SM + 場景包詞表** | 唯一可用。Gemini Live 崩、Scribe 即時輸、地端沒串流介面 |
| 逐字稿・乾淨 | Gemini 批次 | N0 0.940 最高 |
| 逐字稿・吵 | **Scribe 批次** | N3 0.657 最高,且附 timestamp |
| 錯一個詞代價高 | **Gemini + Scribe 並排雙跑** | 重算互補性後換的配對(見下) |
| 中文不可出境 | Breeze + Gemma 4 26B | 已端到端驗證 |

⚠️ **即時線上 SM 是「最後一個站著的」,不是贏來的** ——
而且**韓文與英文的即時準確度本專案一次都沒量過**,只驗證過流程跑得通。
「其他語言先用 SM」是站得住的預設,不是量出來的結論。

### 順帶重算了雙線的最佳配對

| 非即時配對 | 最佳單一 arm(N3) | oracle 天花板 | 增益 |
|---|---|---|---|
| 舊:Gemini 批次 + SM 三方 | 0.597 | 0.731 | +0.134 |
| **新:Gemini 批次 + Scribe 兩方** | **0.657** | 0.746 | +0.090 |

**要看第一欄。** N3 的最佳單一 arm 從 0.597 升到 0.657 ——
那是「只換一個引擎」就拿得到的;+0.090 的 oracle 增益需要事後知道誰對,
真實系統沒有那個信號(鐵律 8)。**換引擎是實得,融合是上限。**

### 這一天的三個更正

1. **我中途報過「即時引擎 +0.103」**——那是控制組只跑 10 個檔的數字,
   跑滿 30 檔是 **−0.075**,**正負號相反**。中途數字不能用來下結論。
2. **我把 SM 控制組並行從 1 改成 2 是錯的**,理由是「exp1 當初用 2 跑成功」——
   那只是看到腳本預設值,不是證據。實測回 `Concurrent Quota Exceeded`,
   **exp5 那條「SM 即時併發上限 1」的侷限是對的**。
3. **發現我們自己 CER metric 的一個偏差**:純中文視窗的參考沒有標點,
   而 CER 把每個標點算成一次插入,系統性懲罰會輸出標點的引擎
   (Scribe 0.333 → 去標點 0.205、Gemini 0.263 → 0.159)。
   **是看到 Scribe 分數之後才發現的**,所以主數字維持原算法,
   去標點那組標成事後敏感度分析(侷限 16r)。
   **受影響的舊結論**:「Breeze 純中文 CER 全場最低」只在含標點的算法下成立。

**花費:批次 $0.45(實付,handoff-v10 §7)+ 即時 ≈ $0.32
(⚠️ **牌價推算**,43.8 分 × ($0.39 realtime + $0.05 keyterms),非帳單金額——
ElevenLabs 的用量 API 只回 TTS 字元數,STT 的實付金額查不到)。
SM 控制組另計在 SM 帳號。

---

## TL;DR — 2026-08-19 這一天做完的事

**十三格實驗、約 $1.9 的 GPU/API 費用,主題是「地端這條線到底能不能用」。**
結論分即時與非即時兩邊講,因為今天做的每一件事都**只影響非即時那一邊**。

### 即時:一個字都沒改,而且今天的結果解釋了為什麼

kikemu 的產線是 **Speechmatics 串流 + Gemini 譯**,今天測的東西**沒有一個能取代它**:

- **今天測的引擎全部是非串流的。** Breeze、Gemma 4 三個型號都是整檔(或切塊)
  單發推論,沒有串流介面、沒有詞級時間戳。**即時字幕根本接不上。**
- **就算不管介面,日文也過不了。** Breeze 在日文導覽上只有 **0.415**
  (SM+詞表 0.791,連沒詞表的 SM 0.639 都輸);Gemma 4 三個型號更低(最好的 E4B 0.328)。
  **kikemu 的本業是日文,地端這條線在本業上是死的。**
- **速度不是瓶頸,可用性才是。** Breeze 在 T4 上跑到 3.80× 實時,吞吐綽綽有餘——
  但**沒有串流 API 就是沒有**,快也沒用。
- **唯一一條即時相關的新證據**:Breeze 在人群 8dB(exp1 N3)**零空輸出、零極短輸出**,
  它一直在講話,只是講錯。對照 Gemini Live 有 2 個空輸出、Gemma 4 有 1 個。
  **失效模式比較安全,但對無人校對的流程,幻覺比空白更難發現。**

> **即時場景的選型沒有變:遠場吵 → Speechmatics 串流 + 詞表 + Gemini 譯。**

### 非即時:這邊全部翻新了

| 問題 | 今天的答案 |
|---|---|
| **資料不能出境的中文逐字稿,能全地端做嗎?** | **可以。** Breeze 聽 + Gemma 4 26B 譯 = 英文術語存活 **0.717**,比雲端兩跳(SM + Gemini)的 0.626 還高 **+0.091**,台灣用語誤用 0、簡體 0 |
| **地端要買什麼卡?** | **T4 就夠。** 每小時音訊 GPU 成本 T4 **$0.155** < L4 $0.175 < A100 $0.412;A100 只快 34% 卻貴 2.7 倍 |
| **地端的數字可信嗎?** | **可跨環境重現。** 三張卡的轉寫**逐字相同**;CPU/fp32 與 T4/fp16 的召回差 **0.000** |
| **Breeze 抗噪是真的嗎?** | **是,而且是全場最平的曲線。** 四條件補齊後掉幅 **+0.070**(SM +0.108、Gemini Live +0.380),中段沒有塌陷 |
| **Gemma 4 能不能取代兩跳?** | **譯可以(便宜 22 倍)、聽只有 E4B 能用且只在中文,接成鏈仍輸 Breeze**(0.518 vs 0.717) |
| **Breeze 能推薦給所有語言嗎?** | **不能。** 它是中文微調,日文 0.415。**地端推薦必須寫上語言** |

**非即時的選型結論:**

- **中文、可出境** → Gemini `generateContent`(28.8× 實時、$0.26/hr,最準最便宜)
- **中文、不可出境** → **Breeze(T4)聽 + Gemma 4 26B 譯**,已端到端驗證
- **非中文、不可出境** → **本專案沒有可推薦的**,不要拿 Breeze 的數字外推
- **錯一個詞代價高** → 兩個引擎都跑、分歧標紅給人看(複核負擔約 11%)

### 今天推翻的三個說法(兩個是我們自己先前寫的)

1. ~~「Gemma 4 不能聽」~~ → **不能聽的是 12B 與 E2B**,E4B 中文可用。
   12B 的低分是**退化生成**(重複迴圈),不是聽力;而且**貪婪解碼下更糟**
   (CER 2.231 → 3.518),先前那個數字其實是隨機取樣「幫過」它的成績。
2. ~~「Breeze 是地端推薦」~~ → **中文可、日文不可**,差 2 倍。
3. ~~「E4B 的轉寫有標點所以下游敢改它」~~ → 這是我今天自己提的機制推測,
   **四個機械變體驗完證明標點與空格都不是主因**;真正原因未知,
   依先寫死的規則**不再猜第三個**。

### 方法上學到的三件事

- **`generation_config.json` 要先看。** Gemma 4 三個型號預設
  `do_sample: true, temperature: 1.0`——不指定就是隨機取樣,同一個檔重跑會不一樣;
  而且**兩個型號方向相反**(12B 貪婪更糟、E4B 更好),不能用通則涵蓋。
- **判讀規則先寫死是有用的,前提是真的照它判。** 今天有三格落在規則沒涵蓋的位置
  (N3 兩端都不完全命中、E4B 兩過一沒過、E2B 的「不吐」不在長度比表裡),
  **都照實記成規則盲點,沒有事後補一格讓它落**。
- **「上限 +X」與「機制看起來很像」都要先驗再寫。** 今天有兩個數字差點寫錯:
  bootstrap 忘了實例加權(正負號會反),以及形式假說(整個被推翻)。

**花費**:v8 梯次一 ≈$0.34、梯次二 ≈$0.75、E2B 與貪婪對照 ≈$0.09、
v9 ≈$0.62、形式驗證 <$0.05,合計 **≈ $1.85**(估算,不是帳單;
Modal CLI 查不到費用,權威在 dashboard)。另有先前**白燒的 $1.26**——
違反模型卡三條限制、以及沒有 chunk 級可見度導致等整個檔跑完才發現壞掉。

---

## 現況(2026-08-19)

**端到端跑通,正式站可用。** 日文與中英夾雜兩條路徑都以評測語料實測驗證過。

| 項目 | 狀態 |
|---|---|
| 聽譯管線(mic → WS → Speechmatics → Gemini 譯) | ✅ 正式站實測通過 |
| 語言:日本語 / 한국어 / English / 中文・English(夾雜) | ✅ 四種皆驗證,預設日文 |
| **場景包:輸入關鍵字自動生成**(Gemini 搜尋接地) | ✅ 正式站日文 149 詞;韓文流程實測 113 詞 |
| 詞條驗證 pipeline(trim → content → reading → dedupe) | ✅ 含既有詞包「重驗」 |
| exp2 追加 arm:Breeze ASR 25(X-breeze) | ✅ Phase A + **exp5 領域外對照**完成(報告 §3D/§3E):優勢被訓練污染放大約 2~3 倍,真正站得住的是抗噪 |
| exp5:Breeze 領域外對照(handoff-v6) | ✅ 3 段 × 2 條件 × 6 arm,**每條件 158 術語實例** |
| exp1 追加 arm `Abat`:Gemini 非 Live 批次 | ✅ 5 條件全覆蓋——證明「噪音崩潰」是 **Live 串流**的行為,不是模型能力上限(報告 §2.1b) |
| oracle 天花板與錯誤互補性(handoff-v7) | ✅ 結論:**不做 GER/融合**,改做非即時的「並排雙跑 + 人工複核」([`oracle_report.md`](results/oracle_report.md)) |
| 案外案:Gemma 4 能不能取代兩跳(報告 §3F) | ✅ **譯可以(1/22 成本);聽只有 E4B 能用,只在中文,而且接成鏈仍輸 Breeze**——三個聽的型號測完,E4B 是甜蜜點(12B 吐太多、E2B 不吐)。[一頁摘要見下](#gemma-4-家族一頁講完五個型號三份語料三輪驗證) |
| handoff-v10 S1~S5(ElevenLabs Scribe) | ✅ **批次很強、即時不行**:批次日文 0.824 / 中英夾雜 0.921(並列第一,N3 **0.657 全場最高**,詞表對齊後**引擎勝 SM +0.096**);**即時引擎輸 SM −0.075,與批次方向相反**——`scribe_v2` 與 `scribe_v2_realtime` 是兩顆不同的模型。**非即時決策樹改版,即時主線不動** |
| handoff-v9 G/H/I/J/K(把 E4B/12B 的坑補完) | ✅ **(G)** E4B 當耳朵的全地端鏈 **0.518 < 0.65** → 耳朵**指定 Breeze**;意外抓到「數據」×14 的兩跳交互作用;**(J)** 12B 貪婪**更糟**(CER 2.231 → 3.518)→ 退化生成是模型性質,結論不用改;E4B 反向(+0.051/+0.038);**(H)** E4B 噪音曲線線性但**掉幅是 Breeze 的 1.8 倍**;**(I)** 日文補滿 335 專名 **0.328** → 「日文不可用」成立;**(K)** E4B − SM batch CI 含 0(同級)、E4B − Breeze CI 不含 0(Breeze 真的較好) |
| handoff-v8 梯次二 C/D/E | ✅ **(C)** Gemma 4 **E4B 比 12B 好一個數量級**(exp5 0.741/0.627 對 0.044/0.013;輸出長度比 1.04 對 2.22)——12B 的低分是**退化生成**不是聽力,「Gemma 4 不能聽」要改成「12B 不能聽」;**(D)** Breeze 每小時音訊 GPU 成本 **T4 $0.155 最便宜**(L4 $0.175、A100 $0.412),且三張卡轉寫**逐字相同**;**(E)** exp1 日文補滿 6 段 335 專名,**0.272**(原 0.323,差 0.051 在門檻內),切塊對 Gemma 的代價 ≈ 0 |
| handoff-v8 梯次一 A/F/B(Modal 上的 Breeze 補測) | ✅ 三格全數完成:**(A)** Breeze 日文只有 0.415,地端推薦**必須限定中文**;**(F)** 全地端聽譯鏈(Breeze 聽 + Gemma 4 譯)端到端驗證成立,0.717 > 雲端兩跳 0.626;**(B)** 噪音曲線四條件補齊,掉幅 +0.070 是全場最平,且 **CPU/fp32 與 T4/fp16 差 0.000**(exp5 侷限 17 解除) |
| 管線狀態列(音量條、計時、逐段診斷) | ✅ |
| 登入、白名單、每日配額、`/admin` | ✅ |
| 本機歷史(IndexedDB)、逐場匯出 MD/TXT/CSV、PWA 安裝、登入前預覽 | ✅ |
| Google OIDC(目前是開發用 Email 直登) | ⬜ 未設定 |
| Turnstile、CANONICAL_HOST | ⬜ 未設定 |
| iOS 真機連續收音驗證 | ⬜ 未做(PRD §8 已列風險) |
| 正式站詞包 | ✅ 大阪城 149 詞(已逐條複驗並重驗修正) |

### Gemma 4 家族:一頁講完(五個型號、三份語料、三輪驗證)

**一句話:譯的那跳可以用而且便宜 22 倍;聽的那跳只有 E4B 能用,只在中文,
而且放進兩跳鏈裡仍然輸給 Breeze。**

Gemma 4 有音訊能力的是 `E2B` / `E4B` / `12B-it`(HF tag `any-to-any`),
**公開 API 一個都拿不到,只能自架**;`26B-A4B` / `31B` 是純文字,反而 API 有。

#### 聽(ASR):中間那個型號最好,兩端都壞,而且壞法不同

| | **E2B** | **E4B** ✅ | **12B** |
|---|---|---|---|
| 模型卡官方英文樣本 | **已經退步** | 完美 | 完美 |
| exp5 中英夾雜(M0/M3 平均) | 0.658 | **0.728**(貪婪) | 0.028 |
| 純中文 20 秒(CER,越低越好) | 0.894(貪婪 **1.000**) | **0.295** | 2.231(貪婪 **3.518**) |
| exp1 日文(sakai 同一子集) | 0.385 | 0.400 | 0.323 |
| 輸出長度 / 參考長度 | 1.03 / **0.35** | **1.04 / 1.17** | **2.22 / 3.48** |
| 失效模式 | **整段放棄**(直接吐 `<eos>`) | 無 | **退化生成**(重複迴圈) |

對照組:Breeze 0.845 / CER 0.198;Speechmatics batch 0.699 / CER 0.206。

**三件事:**

1. **有甜蜜點,不是單調曲線。** 12B 大 3 倍卻差一個量級,E2B 小一半也不行。
   **參數量往大往小都不是能力的代理指標。**
2. **E4B 中文可用**:exp5 與 Speechmatics batch **統計上同級**
   (Δ +0.029,95% CI [−0.037, +0.074] **含 0**),純中文 CER 0.295 也接近
   Gemini 批次的 0.263。但**輸給 Breeze 是穩的**(Δ −0.117,CI 不含 0)。
3. **日文三個都不能用。** E4B 補到 6 段 335 個專名後是 **0.328**,
   對 SM+詞表的 0.791 差得遠。**kikemu 的本業用不到 Gemma 聽。**

#### 譯:這一跳一直是可用的,而且便宜 22 倍

| 第二跳模型 | 專名保留 | 台灣用語 | 簡體 | 延遲 p50 | 成本/hr |
|---|---|---|---|---|---|
| gemini-3.5-flash | 0.522 | 0 | 0 | **2.5s** | $0.112 |
| gemma-4-26b-a4b | 0.522 | 0 | 0 | 7.3s | **$0.005** |
| **gemma-4-31b** | **0.597** | 0 | 0 | 8.4s | **$0.005** |

**品質打平甚至略勝,成本 1/22,但延遲 3 倍。**
kikemu 要即時字幕,所以**不換**;事後逐字稿沒有延遲壓力,換了就是純省。

#### 接成鏈之後:耳朵還是要用 Breeze

| 鏈 | 英文術語存活 | 台灣用語誤用 |
|---|---|---|
| **Breeze → gemma-4-26b(全地端)** | **0.717** | **0** |
| Speechmatics → gemini-3.5-flash | 0.626 | 0 |
| Gemma 4 E4B → gemma-4-26b(純 Gemma) | **0.518** | **14** |

E4B 的 ASR 劣勢**原樣傳到下游**(0.161 的召回差 → 0.180 的存活差)。
而且純 Gemma 那條冒出 14 次「數據」(台灣用「資料」)——
全專案所有鏈第一次出現非零。**我一度以為是轉寫的「形式」害的
(E4B 有標點、Breeze 沒有),四個機械變體驗過之後證明不是**
(標點與空格都不是主因,報告 §3F.4c);真正原因未知,依先寫死的規則不再猜。

#### 用它之前一定要知道的三個坑

1. **`generation_config.json` 預設 `do_sample: true, temperature: 1.0`。**
   不指定就是隨機取樣,同一個檔重跑會不一樣。而且**兩個型號方向相反**:
   12B 改貪婪**更糟**(CER 2.231 → 3.518)、E4B 改貪婪**更好**(+0.05/+0.04)。
2. **模型卡的「限制」章節要讀完**:音訊上限 **30 秒**、audio 必須放在 text
   **之後**、**不要改官方 prompt**(加一句「保留英文」就會把乾淨英文樣本弄壞)。
   違反這三條白燒過 $1.26。
3. **先跑它官方樣本當基準線。** E2B 在 `journal1.wav` 上就已經退步了,
   沒有這條基準線會把「模型不行」誤判成「語料不合」。

原始數據:`exp5/results/scores.json`、`zh_windows_scores.json`、
`chain_scores.json`、`e4b_bootstrap.json`;完整推導見報告 §3F。

### 盤點:哪些解完了,哪些還開著(2026-08-20)

**實驗面已收斂。** 報告的 42 條侷限裡有 8 條被後續實驗解除(原文保留刪節線,
看得出結論怎麼演進);其餘 34 條是**已知且已寫明範圍**的限制,不是待辦。
以下是仍然開著、且**值得再花錢或花時間**的項目:

| 還開著的 | 為什麼還開 | 要做的話 |
|---|---|---|
| **Deepgram 未執行**(侷限 3) | 環境無帳號 | keyterm prompting 上限 100 詞 vs SM 1000 詞,值得比 |
| ~~ElevenLabs Scribe 未執行~~ | **批次與即時都測完**(2026-08-20):批次強(日文 0.824、中英夾雜 0.921,皆並列第一),**但即時引擎輸 SM −0.075,與批次的 +0.096 方向相反** | **主線維持 SM**;非即時線 Scribe 是強候選([`stt-matrix.md`](results/stt-matrix.md) §6b) |
| **Breeze-ASR-**26** 未測** | 我們所有數字都是 **ASR-25** | HF 上已有 26,不要把 25 的 0.845 / 0.415 套上去 |
| **OpenCC 依語言判斷**(app 端) | 評測腳本已依語言,**app 端沒逐欄位檢查過** | 日文欄位全域套 `s2twp` 會把「会・静・図」轉錯([`related-work.md`](docs/related-work.md) §5.1) |
| **SM 泛多語 `multi` pack 未測**(侷限 13) | 免費帳號回 not supported | 現有雙語結論只適用「有專用 pack」的引擎 |
| **prompt 給脈絡能不能取代詞表**(stt-matrix「已知空白」1b) | 沒量過 | LLM 這條路沒有詞表機制,這是唯一對應手段 |
| **三處「無人耳終審」**(侷限 6、11、20、21) | 需要人 | 參考文本、拉丁字母裁決、LLM 是否「補出」內容 |
| **唸稿型中文語料**(侷限 16g) | 需要新語料 | 拆開「語體」與「語言」兩個變因的唯一辦法 |
| ~~Gemma 的取樣變異~~ | **已驗**(v9 J):12B 與 E4B **方向相反** | 剩:E2B 的 exp5/exp1 沒對照(16o) |
| ~~E4B 的噪音曲線與 CI~~ | **已補**(v9 H/K):曲線線性、掉幅 +0.127、CI 已算 | 剩:prompt 變體只有 official |
| ~~E4B 聽 + Gemma 譯沒接過~~ | **已接**(v9 G):0.518,輸給 Breeze 那條的 0.717 | 耳朵維持 Breeze |
| ~~轉寫的「形式」會不會影響下游改寫~~ | **已驗證,答案是否**(v9 L/M):標點與空格四個機械變體都跑過,同來源動 ≤ 0.05、跨來源差一個量級 | 真正原因未知,依先寫死的規則**不再猜** |
| **全地端鏈只驗過中文** | 日文第一跳就垮(Breeze 0.415) | 中文以外沒有可推薦的地端方案 |
| **RNNoise 未測**(exp3) | `pyrnnoise` 與環境 PyAV 不相容 | 已記錄缺口,DSP 那條路整體無效 |

**產品面的待辦不在我這邊**——下面這幾條需要你去設定或用真機驗:

| 待辦 | 誰做 |
|---|---|
| 部署最新版(`cd app && npm run deploy`) | 你 |
| Google OIDC、Turnstile、CANONICAL_HOST | 你(Cloudflare / Google Console) |
| iOS 真機連續收音驗證 | 你(需要實機) |
| AI Studio Spend 頁的供應商端上限 | 你(程式管不到,見 `docs/gemini-api-lessons.md` §保險絲) |
| 把這條分支併回 `main`(目前領先 108 個 commit) | 你決定要不要開 PR |

### 場景包:輸入「大阪城」就出一包

原本要貼官方頁內文,現在在 `/admin` 選語言、填中文別名與關鍵字即可。
兩趟——Gemini **搜尋接地**蒐集固有名詞與讀音(回報搜尋詞與引用來源),
再結構化並按語言驗證讀音字集(日文全形假名 / 韓文諺文)。**先預覽再存**:
讀音錯的詞表會反過來傷辨識,所以會先給你看詞條、來源筆數與警告。
存檔直接收預覽過的詞條,不重跑搜尋(~50ms)。生成軌跡(關鍵字、搜尋詞、來源)
存在包裡可追溯。

這正是 exp1 §2.4 指出但當時走不了的那條路——實驗的詞典為了防洩漏只用維基百科,
對正解專名的字面覆蓋卡在 1/6 ~ 6/7。搜尋能碰到官方頁與在地資料:正式站第一包
「大阪城」跑了 10 次搜尋、引用 10 筆來源,含 **osakacastlepark.jp(官方)**、
**bunka.go.jp(文化廳)**、osaka-info.jp——這些維基百科上沒有。

使用者端只列出**符合當前語言**的包,顯示成「別名・語言(N 詞)」;
語言不符的包伺服器端也會忽略(不會把假名詞條餵給韓文模型)。
**預設不選包**:詞表只在「講的就是這個地點」時有價值,自動掛上清單第一個
等於替使用者猜地點,猜錯就是把一堆不相干的專名推給引擎。

**這條路徑的品質實況(大阪城 149 詞逐條看過)**:142/149 有讀音,涵蓋六座櫓、
巨石名(蛸石、肥後石、振袖石)、大坂冬/夏之陣、年號、建築術語(枡形虎口、
生子壁、鯱瓦)、七個車站與周邊商業設施。同一包裡也有兩個生成瑕疵——
`黄金 of 茶室`(「の」被翻成 of)與 `虎石` 的讀音 `トらいし`(片假名混進平假名)。
兩者都通得過「只看讀音字集」的舊驗證,於是改成四段 **驗證 pipeline**
(`trim → content → reading → dedupe`),每段回報 fix / warn / drop。
正式站那包已按「重驗」修完(不必重跑搜尋):**149 → 149 詞、✎2 ✂0**,
`黄金 of 茶室` → `黄金の茶室`、`トらいし` → `とらいし`,其餘 147 條位置未動,
`source` 生成軌跡原封不動——**fix 是就地改寫,不會少詞**。
修過什麼一定列給管理者看:程式也可能把某個真的叫這個名字的詞「修」壞,
所以「先預覽再存」照樣保留。

### 部署後怎麼複驗

評測語料同時是這個產品的**迴歸測試基準**——已知這批音檔在 exp1 拿 0.836,
所以「有沒有真的生效」是一句指令的事,不是靠感覺:

```bash
cd app
node scripts/probe-ws.mjs --host https://kikemu.ai-apps.work \
  --email clarence.chien@gmail.com \
  --wav ../corpus/conditions/sakai06__N0.wav --lang ja --pack <你的包 id>
```

這支探針到目前抓到的真 bug:

| bug | 症狀 |
|---|---|
| Workers 以 **Blob** 交付二進位 WS 訊息 | 每一框音訊被靜默丟棄;SM 連得上、收得到 EndOfStream,就是一個字都沒有 |
| `cmn_en` 語言包**完全不輸出標點** | 整場累積成一句、到收尾才吐出來,沒有增量字幕 |
| `readPack` 讀取時丟掉 alias / lang | 韓文包在清單上顯示成日文,使用者端過濾不掉 |
| `extractVocab` 的 `JSON.parse` 太嚴格 | 輸出被截斷時整包詞條失敗(與 `make_dict.py` 同一個坑) |

全都是「畫面上看起來只是沒反應」的類型——沒有已知良品音檔會很難查。

---

## 實驗結論(架構的依據)

**一體式模型的「即時介面」只在直接餵訊號的通道可用;只要音訊經過空氣,
即時就要用專用 ASR。而詞表值不值得做,取決於「引擎缺的是什麼」。**

> **2026-08-19 修正一條頭條結論。** 原本寫的是「一體式模型只在直接餵訊號的通道可用」,
> 少了「即時」兩個字。exp1 補跑 `generateContent` 後量到:人聲 8dB 下
> Gemini **Live** 召回 0.030、CER 0.911(6 段中 4 段輸出為空),
> 同一批檔給 **批次** 是 0.567 / CER 0.348,與 SM 的 0.319 同級。
> **崩的是串流路徑,不是模型聽不懂**(報告 §2.1b)。
> 這不改 kikemu 的選型——產品要即時,Live 是即時唯一能用的介面。

| | exp1 日文導覽(專名) | exp2 中英夾雜演講(術語) |
|---|---|---|
| 最佳 arm | Speechmatics + 地點詞表(**Breeze 在日文只有 0.415,不是選項**,§2.1c) | Speechmatics `cmn_en` 雙語 pack(+ 投影片詞表);**但 Breeze ASR 25 無詞表就贏過它,見 §3D;該優勢在領域外縮水約 2~3 倍,見 §3E** |
| **詞表邊際價值** | **+0.12 ~ +0.19(核心資產)** | +0.03 ~ +0.05(小加分) |
| 真正的槓桿 | 詞表 | **語言 pack(+0.23 ~ +0.29)** |
| 一體式乾淨條件 | 打平(0.836 vs 0.836) | 略勝(0.927 vs 0.833,CI 下緣壓 0) |
| 一體式崩潰點 | 人聲 8dB → 0.030(全滅) | 人聲 12dB → 0.396(半滅) |

八條可以直接用的規則:

1. **一體式**即時**的門檻是「直接餵訊號」,不是「安靜」。** exp1 裡只加室內殘響、
   完全不加噪音,Gemini Live 就從 0.836 掉到 0.552,而 SM+詞表維持 0.836。
   **「即時」兩個字是後來補上的**:同一個模型走 `generateContent`(非串流)
   在同一批檔案上是好的——人聲 8dB 下 Live 召回 0.030、批次 0.567,
   CER 0.911 vs 0.348(報告 §2.1b)。崩的是串流路徑,不是模型聽不懂。
2. **詞表投資判準:目標詞彙是「引擎沒見過的」(在地專名、料號)才值得做大;
   「引擎見過但模式不對」(通用英文術語)要先修語言設定。**
3. **選 SM 路線的理由是抗噪與即時性,不是成本。** 兩跳架構的 API 成本
   ~$0.35/hr vs 一體式 ~$0.22/hr(+音訊輸出)——同一數量級,但仍是兩跳比較貴。
   詞表本身很便宜($0.14/景點、一次性)。
   *(2026-08-14 更正:舊版寫 $0.47/hr,那是把 3.5-flash 抄成 flash-lite 價、
   又沒關 thinking 的結果;真價 + thinking 全開其實是 $1.08/hr,關掉思考後
   才是現在的 $0.35。細節見報告 §5。)*
4. **外掛降噪救不了一體式即時,也幫不了 SM**(exp3)。快速前處理(WebRTC NS、
   譜門檻)能讓崩潰的 Gemini Live 重新開口,但召回率是 0;對 SM 零到負。
   吵雜場景的解是換引擎或改拾音物理,不是 DSP。
   (若場景允許非即時,換成 `generateContent` 才是有效的解。)
5. **指向性拾音有效,但紅利歸 SM**(exp4,聲學模擬)。Speak2 級陣列讓
   SM+詞表在 8dB 人聲下 0.597→0.791、領夾麥 0.821;一體式即使配到領夾麥
   也只有 0.507——**SM 裸奔都贏 Gemini Live + 領夾**。正向採購數字需真機驗證。
6. **不要蓋 GER / 多引擎融合**(handoff-v7)。即時場景在最需要幫助的兩格
   天花板只有 **+0.015**;唯一超門檻的非即時 N3(+0.134)不需要 GER
   就能拿到大部分——**兩個引擎都跑、分歧標紅給人看**即可,正常條件複核負擔約 11%。
   同樣的錢花在麥克風上是 +0.194,而且麥克風不會幻覺。
7. **看到「oracle 上限 +X」就問「選錯時的下限是多少」。** 天花板類分析逐項取 OR、
   假設你事後知道誰對;真實系統選錯時會比最佳單一引擎更差。
   oracle 是「值不值得繼續看」的篩選工具,不是「做了會賺多少」的估計。
8. **一個模型的結論要寫上「哪個型號、哪種語言」,寫寬了就是外推。**
   本專案犯過兩次:把 Breeze 的中文數字寫成通用地端推薦(日文實測只有 0.415),
   把 Gemma 4 12B 的失敗寫成「Gemma 4 不能聽」。三個型號都測完之後更清楚:
   **12B 吐太多、E2B 不吐、E4B 剛好**——有甜蜜點,不是單調曲線。
   **參數量不是能力的代理指標,往大往小都不是。**

**這些結論在 app 裡的具體體現:**聽用 Speechmatics 不用一體式(規則 1)、
場景包是核心功能(規則 2)、`getUserMedia` 三個降噪 constraint 全關(規則 4)、
開始聽時提示「貼近音源」(規則 5)、中文用 `cmn_en` 雙語包不用單語 `cmn`(exp2)。

## 文件

| 檔案 | 內容 |
|---|---|
| [`docs/PRD.md`](docs/PRD.md) | **產品規格**:畫面、設計系統、音訊管線、安全基線、配額、已知風險 |
| [`app/README.md`](app/README.md) | **部署 runbook**、架構圖、診斷流程(出不來字時怎麼查) |
| [`results/report.md`](results/report.md) | **完整評測報告**(五個實驗合併),方法、數據、**42 條侷限**(8 條已由後續實驗解除,原文保留刪節線) |
| [`results/oracle_report.md`](results/oracle_report.md) | **Oracle 天花板與錯誤互補性**:融合/GER 值不值得做,以及「並排雙跑」怎麼算 |
| [`results/stt-matrix.md`](results/stt-matrix.md) | 跨專案 **STT 選型決策矩陣**——照情境查該用什麼 |
| [`docs/related-work.md`](docs/related-work.md) | **同類專案掃描**(2026-08):離線字幕工具的做法,以及「即時場景怎麼改」——多數結論相反。含建議行動順序 |
| [`docs/gemini-api-lessons.md`](docs/gemini-api-lessons.md) | **Gemini API 教訓**:thinking 稅 A/B 實測、成本表更正、保險絲四層現況 |
| [`CLAUDE.md`](CLAUDE.md) | 接手須知:這個 repo 的九條鐵律與驗證方式 |
| [`notebooks/README.md`](notebooks/README.md) | **Colab 操作手冊**:Breeze ASR 25(X-breeze arm)一鍵開跑 |
| [`handoff.md`](handoff.md) ~ [`handoff-v9.md`](handoff-v9.md) | 九份實驗任務書(v5 = X-breeze Phase A;v6 = 領域外對照;v7 = oracle 天花板;**v8 = Modal 上的 Breeze/Gemma 補測(A/F/B + C/D/E,全部完成);v9 = 把 E4B/12B 的坑補完(G/H/I/J/K,全部完成)**。v6~v9 任務書末尾都有執行結果與偏離紀錄) |

## 實驗規模

| | exp1 日文導覽 | exp2 中英演講 | exp3 前處理 | exp4 指向性 | exp5 領域外 |
|---|---|---|---|---|---|
| 語料 | 大阪觀光局 6 段 | 李宏毅課程 3 段 × 5 分 | 同 exp1 | 同 exp1/exp2 | 台灣科技 podcast 3 段 × 5 分 |
| 正解 | 67 個專有名詞 | 86 個英文術語實例 | 同 exp1 | 同左 | 158 個英文術語實例/條件 |
| 追加 | Gemma 4 日文組 `Xgma_ja`(6 段)/ `_e4b` / `_e2b`;Breeze 日文 `Xbrz_ja` | — | — | — | 純中文 20 秒視窗 × 8(CER,含貪婪對照);Gemma 4 E4B / E2B;Breeze 四條件曲線 |
| 聲學條件 | 5 | 4 | 4 × 3 前處理 | 3 profile × 3 條件 | 2(M0/M3 兩端) |
| arm | 10(含 `Abat`、`Xbrz_ja`) | 6 | 2 引擎 × 3 前處理 | 2 引擎 × 3 profile | 9(含 `Xgma_e4b`/`e2b`、`Xbrz_gpu`) |

總支出 < $10,主要成本是真實速度推流的 wall-clock。

外加一份 **oracle 天花板分析**(handoff-v7,[`results/oracle_report.md`](results/oracle_report.md)):
第一輪 **0 元**——直接用既有的 per-item outcome 檔算完,沒有重跑任何 arm。

## 信度控制

這份評測的價值在於數字可信,做法如下:

- **標準答案不靠單人聽寫。** exp1 用雙引擎交叉驗證官方頁面文字(三個已知陷阱全部命中並處理);
  exp2 沒有官方文字,改用三路轉寫 + 80 處分歧逐條人工裁決
- **判定規則執行前凍結。** strict / tolerant 兩套並列寫死在腳本,不臨場判斷
- **洩漏控制。** 詞表生成絕不接觸作為標準答案來源的文本
- **真實材料。** RIR 取自 MIT IR Survey 實測、噪音取自 DEMAND 實錄,固定種子可重現
- **真實速度。** 所有即時 arm 以 1× 推流,訊息級時戳落檔,延遲與改寫率由此計算
- **統計。** 配對設計 + segment-cluster bootstrap 95% CI + McNemar
- **重現性驗證。** exp1 一體式的崩潰做過獨立重跑確認逐檔重現,
  並在補跑 `generateContent` 後查明那是**串流路徑**的行為而非模型能力上限
- **正確性檢查。** oracle 分析的每個單一 arm 數字都回頭比對既有報告的召回率,全部相符
- **誠實記錄。** 42 條侷限寫進報告,含未執行的 arm 與環境限制造成的替代方案。
  解除的 8 條**不刪掉**,改成刪節線並註明是哪一次實驗解除的——這樣看得出結論怎麼演進的。
  數字對不上就修:exp5 的術語實例數曾誤寫 128,對回 `term_outcomes.json` 是 158,已全面更正

## 目錄

```
app/                產品本體(Cloudflare Workers + vanilla TS + Vite PWA)
  worker/           OIDC、白名單、配額 DO、Speechmatics relay DO、場景包、admin
    langs.ts        語言清單唯一來源(下拉、relay 設定、詞表驗證字集都讀它)
  src/ public/      單頁 UI、AudioWorklet、PWA
  scripts/probe-ws.mjs   端到端探針(拿評測語料當迴歸測試)
docs/               PRD.md(產品規格)、gemini-api-lessons.md(Gemini 教訓)
scripts/ corpus/ results/    exp1・exp3・exp4(腳本、語料 metadata、原始回應)
analysis/           oracle 天花板與錯誤互補性(純計算,不呼叫任何 API)
exp2/               exp2(中英夾雜);corpus/audio_manifest.json 是音檔指紋
exp5/               領域外對照(handoff-v6);corpus/audio_manifest.json 是音檔指紋
notebooks/          需要 GPU 的 arm(Breeze ASR 25,Colab)
```

音檔依授權不重新散布(`.gitignore`),但抓取與切片腳本可完整重現。
