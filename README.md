...
| 案外案:Gemma 4 能不能取代兩跳(報告 §3F) | ✅ **譯可以(1/22 成本);聽要看型號**——12B 不行(CER 2.231,退化生成),**E4B 中文可用**(CER 0.312、exp5 0.684,與 SM batch 同級),日文兩者都不行 |
...
8. **一個模型的結論要寫上「哪個型號、哪種語言」,寫寬了就是外推。**
   ...
   把 Gemma 4 12B 的失敗寫成「Gemma 4 不能聽」(同家族 E4B 在中文上是可用的,
   而且大 3 倍的 12B 差它 17 倍)。**參數量不是能力的代理指標。**
...
| 追加 | Gemma 4 日文組 `Xgma_ja`(6 段)/ `Xgma_ja_e4b`;Breeze 日文 `Xbrz_ja` | — | — | — | 純中文 20 秒視窗 × 8(CER);Gemma 4 E4B;Breeze 四條件曲線 |
| 聲學條件 | 5 | 4 | 4 × 3 前處理 | 3 profile × 3 條件 | 2(M0/M3 兩端) |
| arm | 9(含 `Abat`、`Xbrz_ja`) | 6 | 2 引擎 × 3 前處理 | 2 引擎 × 3 profile | 8(含 `Xgma_e4b`、`Xbrz_gpu`) |
